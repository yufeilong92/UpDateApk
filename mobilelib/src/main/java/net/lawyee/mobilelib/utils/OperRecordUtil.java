...
public class OperRecordUtil
{
	protected static final String TAG = OperRecordUtil.class.getSimpleName();
	
	/**
	 * 操作记录文件夹
	 */
	private static String CSTR_OPERRECORDFILE = Constants.getDataStoreDir()+"operrecord.dat";
	
	/**
	 * 
	 */
	private static OperRecordUtil operRecordUtil;
	...
	private void initOperRecordDataList()
	{
		...
		File favfile = new File(CSTR_OPERRECORDFILE);
	...
	private void saveOperRecordDataList()
	{
		...
		try
		{
			FileUtil.isExistFolderFromFile(CSTR_OPERRECORDFILE); // 文件夹存在与否检测，不存在则创建
			File f = new File(CSTR_OPERRECORDFILE);
	...
	public static OperRecordUtil getUtil()
	{
		if(operRecordUtil==null)
			operRecordUtil=new OperRecordUtil();
		return operRecordUtil;
	}
